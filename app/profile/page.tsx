...
function ProgressCircle({ pct }: { pct: number }) {
  ...
  return (
    <svg width="88" height="88" viewBox="0 0 88 88" className="shrink-0">
      ...
      <circle cx="44" cy="44" r={r} fill="none" stroke={done ? "#4ade80" : "#f59e0b"} strokeWidth="4"
...
function ProfileContent() {
  ...
  async function handleSave() {
    setSaving(true);
    const result = await updateProfile({ username: username || null, displayName: displayName || null });
    setSaving(false);
    flash(result.error ?? "Changes saved.", !!result.error);
  }
        ...
        {activeTab === "Profile" && (
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-1.5">
              ...
              <input disabled value={email} className="bg-[#18181c] border border-[#2e2e38] rounded-lg px-4 py-3 text-sm text-[#7a7870] cursor-not-allowed" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-[#7a7870] tracking-widest uppercase">Display Name</label>
