import { NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { createClient as createServiceClient } from "@supabase/supabase-js";
import { resolveContext } from "@/lib/context";

function db() {
  return createServiceClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!
  );
}

export async function POST(request: Request) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { name, icon, asset_type, current_value, is_liquid, linked_debt_id, context_type, context_id } = await request.json();

  if (!name || current_value == null) {
    return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
  }

  const ctx = await resolveContext(context_type, context_id, user.id);
  if (!ctx) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  // Enforce no double-linking: check the debt isn't already claimed by another asset
  if (linked_debt_id) {
    const { data: existing } = await db()
      .from("assets")
      .select("id")
      .eq("linked_debt_id", linked_debt_id)
      .eq("owner_type", ctx.type)
      .eq("owner_id", ctx.id)
      .maybeSingle();
    if (existing) {
      return NextResponse.json({ error: "This debt is already linked to another asset." }, { status: 409 });
    }
  }

  const { data, error } = await db()
    .from("assets")
    .insert({
      owner_id:       ctx.id,
      owner_type:     ctx.type,
      name,
      icon:           icon           ?? "💎",
      asset_type:     asset_type     ?? "other",
      current_value:  Number(current_value),
      is_liquid:      Boolean(is_liquid ?? false),
      linked_debt_id: linked_debt_id ?? null,
    })
    .select("id")
    .single();

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ asset_id: data.id });
}
