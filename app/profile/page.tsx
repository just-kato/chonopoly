  ...
  return (
    <div className="min-h-screen bg-[#0f0f11] text-[#e8e6df]">
      <div className="bg-[#18181c] border-b border-[#2e2e38] px-6 py-4">
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-[#7a7870] hover:text-amber-400 text-sm transition-colors"
        >
          <ArrowLeft size={14} />
          Back to Study Guide
        </Link>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-4 mt-1">
              ...
              <form action={logout}>
                <button
                  type="submit"
                  className="text-sm text-[#7a7870] hover:text-red-400 transition-colors"
                >
                  Sign out
                </button>
              </form>
          ...
          <div>
            ...
            <div className="bg-[#18181c] border border-[#2e2e38] rounded-xl p-6 mb-8">
              <div className="flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h2 className="text-white font-serif text-lg font-bold leading-tight">
            ...
            <div className="flex flex-col gap-5">
              {Object.entries(groups).map(([group, chs]) => (
                <div key={group}>
                  <p className="text-[#7a7870] font-mono text-[10px] tracking-widest mb-2 uppercase">
                    Chapter {group}
                  </p>
                  <div className="flex flex-col gap-1">
                    {chs.map((ch) => {
                      ...
                      return (
                        ...
                        >
                          <span className="font-mono text-[11px] text-[#7a7870] shrink-0">
                            {ch.chapterNumber}
                          </span>
                          <span className="flex-1 text-sm text-[#c8c5bc] group-hover:text-white transition-colors truncate">
                            {ch.title}
                          </span>
                          {complete ? (
                            <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0" />
                          ) : (
                            <span className="w-2 h-2 rounded-full bg-[#2e2e38] shrink-0" />
                          )}
                        </Link>
                      );
                    })}
