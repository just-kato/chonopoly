  ...
  const load = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        const periodEnd = new Date(new Date(y, m, startDay).getTime() - 86_400_000);
        ...
        const updated = (json.summaries ?? []).find(
          (s: BudgetSummary) => s.budget_id === budgetId
        );
      ...
      ListHeaderComponent={
        <View>
          <View
            style={[
              styles.headerCard,
              { borderColor: over_budget ? "#ef4444" : category_color + "55" },
            ]}
          >
            <View style={[styles.iconBadge, { backgroundColor: category_color + "22" }]}>
              <Text style={styles.iconText}>{category_icon}</Text>
            </View>
            ...
            <View style={styles.statsRow}>
              <View style={styles.stat}>
                ...
                <Text style={[styles.statValue, over_budget && { color: "#ef4444" }]}>
              ...
              <View style={styles.stat}>
                ...
                <Text
                  style={[
                    styles.statValue,
                    { color: over_budget ? "#ef4444" : "#34d399" },
                  ]}
                >
                  $
                  {over_budget
            ...
            <ProgressBar percent={percent_used} over={over_budget} />
...
const styles = StyleSheet.create({
  ...
  txRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 14, paddingHorizontal: 4 },
  txMeta: { flex: 1 },
